...
public class CategoryService {
	...
	public ListCategory getCategoryById(long id) {
		...
		ListCategory categ= this.categoryRepository.findById(id)
				.map(c->new ListCategory(c.getCategoryId(), c.getName(), c.getParent(),
						this.categoryRepository.findAll().stream()
						.filter(cat-> cat.getParent()!=null && cat.getParent().getCategoryId().equals(c.getCategoryId()))
